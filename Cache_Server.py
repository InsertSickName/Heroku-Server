...
try:
    ...
    host_ip = ip
    port = 9669
    out_log("Client Serving Server")
    out_log("Host IP:", host_ip)
    server_socket.bind((host_ip, port))
    server_socket.listen()
    out_log("Listening at:", (host_ip, port), "\n\n")
    ...
    def serve_client(addr, client_socket):
        global frame
        try:
            out_log(f"Client {addr} Connected!")
            if client_socket:
                while True:
            ...
            out_log(f"Client {addr} Disconnected!")
